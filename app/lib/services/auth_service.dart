...
class AuthService extends ChangeNotifier {
    ...
    Future<void> initialize() async {
        ...
        // If already authenticated with a known session token, skip full re-init
        ...
        try {
            ...
            AppLogger.debug('Auth initialization - Session token present: ${_sessionToken != null}');
    ...
    Future<void> completeNormalAuth() async {
    ...
    Future<void> completeDesktopAuth() async {
        return await completeAuth();
    }
        ...
        for (int i = 0; i < 150; i++) {
            ...
            try {
                ...
                if (response.statusCode == 200) {
                    ...
                    if (authStatus.status == 'completed' && authStatus.sessionToken != null) {
                        await setSessionToken(authStatus.sessionToken!);
                        return;
                    }
                }
            }
    ...
    Future<void> logout() async {
        ...
        // Invalidate token on server before clearing local data
        if (_sessionToken != null) {
            try {
                await http.post(
                    Uri.parse(AppConfig.apiUrl('/auth/logout')),
                    headers: {'X-Auth-Token': _sessionToken!},
                );
            } catch (e) {
                AppLogger.debug('Server logout call failed (continuing): $e');
            }
        }
