...
const rateLimitCheck = async (sessionToken) => {
...
3. Session token issued and returned
...
const UserProfile = ({ sessionToken }) => {
...
const generatePlaylist = async (sessionToken, prompt) => {
  try {
